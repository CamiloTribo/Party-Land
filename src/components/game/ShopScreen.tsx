...
const TOKEN_SKINS = [
  ...
  { id: 'inspector', name: 'Inspector Pink', cost: 150, description: 'Detective mode' },
  { id: 'miami', name: 'Miami Pink', cost: 400, description: 'Very cool' },
  { id: 'golden', name: 'Golden Panther', cost: 750, description: 'Pure luxury' },
  { id: 'ninja', name: 'Ninja Pink', cost: 1200, description: 'Stealth mode' },
  { id: 'cyber', name: 'Cyber Pink', cost: 1500, description: 'From future' },
  { id: 'royal', name: 'Royal Pink', cost: 2200, description: 'Royalty' },
  { id: 'space', name: 'Space Pink', cost: 3000, description: 'Cosmic' },
  { id: 'vampire', name: 'Vampire Pink', cost: 4000, description: 'Night hunter' },
  { id: 'pirate', name: 'Pirate Pink', cost: 5500, description: 'Sea raider' },
  { id: 'wizard', name: 'Wizard Pink', cost: 6500, description: 'Magical' },
  { id: 'knight', name: 'Knight Pink', cost: 8000, description: 'Brave knight' },
  { id: 'samurai', name: 'Samurai Pink', cost: 12000, description: 'Warrior' },
  { id: 'robot', name: 'Robot Pink', cost: 15000, description: 'Tech AI' },
  { id: 'diamond', name: 'Diamond Pink', cost: 22000, description: 'Precious' },
  { id: 'legendary', name: 'Legendary Pink', cost: 40000, description: 'Ultimate' },
];
...
const USDC_SKINS = [
  ...
  { id: 'joker', name: 'Pink Joker', cost: 0.023, description: 'Why so pink? 🃏', isOffer: true, originalCost: 2.3 },
  ...
  // Special Edition
  { id: 'farcaster', name: 'FC Pink', cost: 0.23, description: 'Web3 social legend ✦', isNew: true },
];
...
export default function ShopScreen({ onBack, soundEnabled, onToggleSound }: ShopScreenProps) {
  ...
  const handlePurchaseSuccess = (txHash?: string) => {
    ...
    if (selectedItem) {
      buySkin(selectedItem.id, selectedItem.cost, txHash).then(() => {
        selectSkin(selectedItem.id);
        refetchUSDC();
        setModalType('success');
      });
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-b from-purple-900 via-purple-800 to-purple-900 overflow-y-auto">
      ...
      {/* Skins Grid */}
      {/* Grid de Items */}
      <div id="tutorial-shop-skins-grid" className="flex-1 overflow-y-auto px-4 pb-20">
              ...
              >
                ...
                {shopType === 'usdc' && (
                  <div className="absolute -top-2 -right-2 z-10">
                    {(skin as any).isOffer ? (
                      <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-purple-900 text-[9px] font-black px-2 py-0.5 rounded-full shadow-lg border border-white/30 flex items-center gap-0.5 animate-pulse">
                        ⚡ OFFER
                      </div>
                    ) : (
