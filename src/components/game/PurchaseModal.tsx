...
import { X, Share2, Coins, Zap, CheckCircle2, Trophy, Ticket, DollarSign, Lock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import PinkPantherPlayer from '../PinkPantherPlayer';
import { Button } from '~/components/ui/Button';
import { shareToFarcaster, attachBuilderCode } from '~/lib/utils';
import { useState, useEffect } from 'react';
import { useAccount, useSendTransaction, useWaitForTransactionReceipt, useSwitchChain } from 'wagmi';
import { parseUnits } from 'viem';
import { PAYMENT_WALLET_ADDRESS, USDC_ADDRESS, PAYMENT_CHAIN_ID, BUILDER_CODE } from '~/lib/constants';
...
interface PurchaseModalProps {
    ...
    currentBalance?: number;  // USDC balance
    tokenBalance?: number;    // Game token balance (for airdrop dual-cost)
    tokenCost?: number;       // Tokens to burn (for airdrop dual-cost)
    bgColor?: string;
...
export const PurchaseModal = ({
    ...
    const isAirdropEntry = item.id.startsWith('airdrop');
    ...
    // NO AUTO-CLOSE on success to allow user to see result and Share
    useEffect(() => {
        if (isConfirmed && txHash && purchaseState !== 'success') {
            setPurchaseState('success');
            fireConfetti();
            onSuccess?.(txHash); // Trigger callback with hash
        }
    }, [isConfirmed, txHash, onSuccess, purchaseState]);
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (isUSDC) {
                ...
                const txData = attachBuilderCode(encodeFunctionData({
                    abi: USDCABI,
                    functionName: 'transfer',
                    args: [PAYMENT_WALLET_ADDRESS, amount],
                }));

                console.log('📝 [PurchaseModal] Transaction Data:', {
                    to: USDC_ADDRESS,
                    data: txData,
                    attributionCode: BUILDER_CODE,
                    suffix: txData.slice(-40) // Show only the end for verification
                });
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center text-center">
                            {/* Character Icon or Airdrop Icon */}
                            <div className="mb-6 relative">
                                {type === 'success' || purchaseState === 'success' || isConfirmed ? (
                                    <div className="flex flex-col items-center gap-4">
                                        {isAirdropEntry ? (
                                            <div className="w-20 h-20 rounded-full bg-violet-600/20 border-2 border-violet-400/40 flex items-center justify-center shadow-xl shadow-violet-500/30">
                                                <Zap className="w-10 h-10 text-violet-400" strokeWidth={2} />
                                            </div>
                                        ) : bgColor ? (
                                    ...
                                    bgColor ? (
                                        ...
                                        isAirdropEntry ? (
                                            <div className="w-20 h-20 rounded-full bg-violet-600/20 border-2 border-violet-400/40 flex items-center justify-center shadow-xl shadow-violet-500/20">
                                                <Ticket className="w-9 h-9 text-violet-400" strokeWidth={1.5} />
                                            </div>
                                        ) : (
                                            <div className="w-28 h-28 flex items-center justify-center relative">
                                                <div className="transform scale-[2]">
                            ...
                            {/* Title */}
                            <h3 className={`text-3xl font-black uppercase mb-3 ${isAirdropEntry ? 'text-violet-400' : isUSDC ? 'text-blue-400' : 'text-[#ff69b4]'}`}>
                                {purchaseState === 'success' || isConfirmed
                                    ? (isAirdropEntry ? 'Entered!' : 'Unlocked!')
                                    : purchaseState === 'pending' || isSendingTx || isConfirming
                                        ? 'Processing...'
                                        : 'Confirm Purchase'}
                            </h3>
                            ...
                            {purchaseState !== 'success' && !isConfirmed ? (
                                <div className="w-full mb-8">
                                    {/* AIRDROP: dual cost summary — USDC + Tokens */}
                                    {isAirdropEntry ? (
                                        <div className="space-y-3">
                                            {/* USDC block */}
                                            <div className="bg-blue-950/40 rounded-xl px-4 py-3 border border-blue-500/20">
                                                <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mb-2 flex items-center gap-1.5">
                                                    <DollarSign className="w-3 h-3" /> USDC Payment · Base
                                                </p>
                                                <div className="space-y-1.5">
                                                    <div className="flex justify-between text-sm">
                                                        <span className="text-white/60">Cost</span>
                                                        <span className="text-white font-bold">${item.cost} USDC</span>
                                                    </div>
                                                    <div className="flex justify-between text-sm">
                                                        <span className="text-white/60">Your balance</span>
                                                        <span className="text-white font-bold">${currentBalance.toFixed(4)} USDC</span>
                                                    </div>
                                                    <div className="border-t border-white/10 pt-1.5 flex justify-between text-sm">
                                                        <span className="text-white/60">After entry</span>
                                                        <span className={`font-bold ${canAfford ? 'text-green-400' : 'text-red-400'}`}>
                                                            ${Math.max(0, remainingBalance).toFixed(4)} USDC
                                                        </span>
                                                    </div>
                                                </div>
                                                {!canAfford && (
                                                    <p className="text-red-400 text-xs mt-2 flex items-center gap-1">
                                                        <Lock className="w-3 h-3" /> Insufficient USDC balance
                                                    </p>
                                                )}
                                            </div>
                                            {/* Token block */}
                                            <div className="bg-yellow-950/30 rounded-xl px-4 py-3 border border-yellow-500/20">
                                                <p className="text-[10px] text-yellow-400 font-bold uppercase tracking-widest mb-2 flex items-center gap-1.5">
                                                    <Coins className="w-3 h-3" /> Game Tokens · Burned
                                                </p>
                                                <div className="space-y-1.5">
                                                    <div className="flex justify-between text-sm">
                                                        <span className="text-white/60">Cost</span>
                                                        <span className="text-white font-bold">1,500 tokens</span>
                                                    </div>
                                                    <div className="flex justify-between text-sm">
                                                        <span className="text-white/60">Your balance</span>
                                                        <span className="text-white font-bold">{tokenBalance.toLocaleString()} tokens</span>
                                                    </div>
                                                    <div className="border-t border-white/10 pt-1.5 flex justify-between text-sm">
                                                        <span className="text-white/60">After entry</span>
                                                        <span className={`font-bold ${tokenBalance >= 1500 ? 'text-green-400' : 'text-red-400'}`}>
                                                            {Math.max(0, tokenBalance - 1500).toLocaleString()} tokens
                                                        </span>
                                                    </div>
                                                </div>
                                                {tokenBalance < 1500 && (
                                                    <p className="text-red-400 text-xs mt-2 flex items-center gap-1">
                                                        <Lock className="w-3 h-3" /> Need {(1500 - tokenBalance).toLocaleString()} more tokens
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                    ) : (
                                        /* NORMAL: single cost block */
                            ...
                            {purchaseState !== 'success' && !isConfirmed ? (
                                ...
                                <div className="flex flex-col gap-3 w-full">
                                    {isAirdropEntry ? (
                                        <>
                                            <div className="w-full py-3 px-4 bg-violet-900/40 border border-violet-500/30 rounded-2xl text-center">
                                                <p className="text-violet-300 text-sm font-bold">You're in {item.name}!</p>
                                                <p className="text-violet-400 text-xs mt-0.5">Check the Airdrop panel for your payout details</p>
                                            </div>
                                            <Button
                                                onClick={() => { soundManager.play('click'); onClose(); }}
                                                className="w-full bg-gradient-to-r from-violet-600 to-purple-700 text-white font-bold flex items-center justify-center gap-2"
                                            >
